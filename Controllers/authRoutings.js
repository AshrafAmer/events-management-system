    ...
    switch (userData.userName) {
        case "admin":
            if (userData.password == "123") {
                ...
                response.redirect("speakers/admin");
            } else {
                request.flash("invalidPass", "Invalid Password.!");
                response.render("login/login.ejs", {
                    error: request.flash("invalidPass"),
                    userName: "admin"
                });
            }
            break;
            ...
            users
                ...
                .then(data => {
                    ...
                    if (data.length == 0) {
                        ...
                        response.render("login/login.ejs", {
                            error: request.flash("passNotCorrect"),
                            userName: userData.userName
                        });
                    } else {
                        ...
                        response.redirect("/speakers/profile/" + data[0]._id);
                ...
                .catch(error => {
                    ...
                    response.render("login/login.ejs", {
                        error: request.flash("dataBaseError"),
                        userName: userData.userName
                    });
                });
            break;
...
authRoutings.get("/register", (request, response) => {
    users
        .findOne({}, { _id: 1 })
        .sort("-_id")
        .then(data => {
            response.render("register/register.ejs", {
                _idValue: data._id + 1,
                RegisterError: ""
            });
        })
